...
function loopless_fba_vs_cb(organisms; cuts=true)
    ...
    for organism in organisms
        ...
        if dict["termination"] == "INFEASIBLE"
        ...
        if isnothing(dict_organism[:objective_value_ll_fba_cobrexa])
            dict_organism[:objective_value_ll_fba_cobrexa] = NaN 
        end

        if isnothing(dict_organism[:objective_value_ll_fba_nullspace])
            dict_organism[:objective_value_ll_fba_nullspace] = NaN 
        end
        @show dict_organism
...
loopless_fba_vs_cb(organisms, cuts=true)
