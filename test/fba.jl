...
@testset "simple model" begin
    ...
    feasible, _, _e = thermo_feasible_mu(internal_rxn_idxs, internal_flux_directions, S)
    @show feasible
    @test !feasible
...
@testset "iAF692" begin
    ...
    # test ll-ness through internal reactions
    internal_flux_directions = [vars_fba[idx] >= 1e-5 ? 1 : 0 for idx in internal_rxn_idxs]
    feasible, _, _ = thermo_feasible_mu(internal_rxn_idxs, internal_flux_directions, S)
    @show feasible
    @test !feasible
