...
@testset "simple model with one loop" begin 
    println("--------------------------------------------------------")
    println("TEST SIMPLE MODEL WITH ONE LOOP")
    println("--------------------------------------------------------")
    ...
    flux_directions = [solution[idx] >= 1e-5 ? 1 : 0 for idx in internal_rxn_idxs]
    thermo_feasible = thermo_feasible_mu(internal_rxn_idxs, flux_directions, S)    
    ...
    # check thermodynamic feasibility of solution
    flux_directions = [solution[idx] >= 1e-5 ? 1 : 0 for idx in internal_rxn_idxs]
    @test !thermo_feasible_mu(internal_rxn_idxs, flux_directions, S) # due to directionality of 0 flux
...
@testset "simple model" begin
    println("--------------------------------------------------------")
    println("TEST SIMPLE MODEL WITH TWO LOOPS")
    println("--------------------------------------------------------")
