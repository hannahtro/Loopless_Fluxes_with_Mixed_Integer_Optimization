...
# COBREXA function with inequality constraints
function make_optimization_model_ineq_form(model::MetabolicModel, optimizer; sense = MAX_SENSE)
    ...
    @constraint(optimization_model, mb_ineq, -stoichiometry(model) * x .<= -balance(model)) # mass balance
