...
# FBA data 
function cobrexa_fba_data(organism; optimizer=SCIP.Optimizer, time_limit=1800, mute=true, json=true, enzyme_data=false, seed=10)
    # build model
    ...
    # write_to_file(model, "../experiments/csv/models/my_gecko_" * organism * ".lp")
...
# loopless FBA data
function cobrexa_loopless_fba_data(organism; optimizer=SCIP.Optimizer, time_limit=1800, mute=true, json=true, enzyme_data=false, seed=10)
    # build model
    molecular_model = load_model(StandardModel, "../molecular_models/" * organism * ".json")
    # print_model(molecular_model, organism)
    if enzyme_data
