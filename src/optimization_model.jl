...
"""
build disjunctive FBA model
"""
function build_disjunctive_fba_model(S_transform, lb_transform, ub_transform; set_objective=false, optimizer=SCIP.Optimizer, time_limit=1800)
    # make optimization model
    ...
    @variable(optimization_model, x[1:n])
    for i in 1:n
        set_lower_bound(x[i], lb_transform[i])
        set_upper_bound(x[i], ub_transform[i])
    end
    @constraint(optimization_model, mb, S_transform * x .== 0) # mass balance
    # @show optimization_model
