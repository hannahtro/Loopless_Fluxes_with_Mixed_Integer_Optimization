...
"""
overwrite COBREXA method to set SCIP seed
"""
function make_optimization_model(model::MetabolicModel, optimizer; sense = MAX_SENSE, scip_seed=0)
    ...
    if optimizer == SCIP.Optimizer
        optimization_model = Model(
            optimizer_with_attributes(optimizer, "randomization/randomseedshift" => scip_seed)
        )
    else 
        optimization_model = Model(optimizer)
    end
