...
function build_fba_indicator_model_moi(S_transform, lb_transform, ub_transform, internal_rxn_idxs; set_objective=false, optimizer=SCIP.Optimizer, time_limit=Inf, objective_func_vars=Nothing, objective_func_coeffs=Nothing, silent=true, big_m=true)
    # make optimization model
    optimization_model = Model(optimizer)
    ...
    a = build_master_problem(model, internal_rxn_idxs, big_m=big_m)
