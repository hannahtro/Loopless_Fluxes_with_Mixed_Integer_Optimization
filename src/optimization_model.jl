...
"""
build FBA model inequality form
"""
function build_fba_model_inequalitiy_form(S_transform, lb_transform, ub_transform; set_objective=false, optimizer=SCIP.Optimizer, time_limit=1800)
    # make optimization model
    optimization_model = Model(optimizer)
    _, n = size(S_transform)
    # @show size(S_transform)
    ...
    @variable(optimization_model, x[1:n])
    @constraint(optimization_model, mb, S_transform * x .<= 0) # mass balance
    @constraint(optimization_model, mb_ineq, - S_transform * x .<= 0) # mass balance
    @constraint(optimization_model, lbs, -x .<= - lb_transform) # lower bounds
    @constraint(optimization_model, ubs, x .<= ub_transform) # upper bounds
    # set_lower_bound.(x, lb_transform)
    # set_upper_bound.(x, ub_transform)
    # @show optimization_model
...
"""
build FBA model in standard form
"""
function build_fba_model_standard_form(S, lb, ub; set_objective=false, optimizer=SCIP.Optimizer, time_limit=1800)
    # make optimization model
    optimization_model = Model(optimizer)
    _, n = size(S)
    # @show size(S_transform)
    ...
    @variable(optimization_model, x_pos[1:n]) # positive flux
    @variable(optimization_model, x_neg[1:n]) # negative flux
    @variable(optimization_model, s_lb[1:n]) # slack variables
    @variable(optimization_model, s_ub[1:n]) # slack variables

    @constraint(optimization_model, mb, S * (x_pos - x_neg) .== 0) # mass balance
    @constraint(optimization_model, lbs, -x_pos + x_neg + s_lb .== - lb) # lower bounds
    @constraint(optimization_model, ubs, x_pos - x_neg + s_ub .== ub) # upper bounds
    
    @constraint(optimization_model, x_pos .>= 0)
    @constraint(optimization_model, x_neg .>= 0)
    @constraint(optimization_model, s_lb .>= 0)
    @constraint(optimization_model, s_ub .>= 0)

    # @show optimization_model

    if set_objective
        @objective(optimization_model, Max, sum(x_pos - x_neg))
    end 
