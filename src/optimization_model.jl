...
function internal_reactions(molecular_model; enzyme_data=false)
    if enzyme_data 
        reaction_names = [string(first(split(rid, "#"))) for (ridx, rid) in enumerate(reactions(molecular_model))]
        original_reaction_names = [haskey(molecular_model.inner.reactions, rid) for rid in reaction_names]
    
        internal_rxn_idxs = Int64[]
        for (ridx, rid) in enumerate(reactions(molecular_model))
            if original_reaction_names[ridx]
                if !is_boundary(molecular_model, string(first(split(rid, "#"))))
                    push!(internal_rxn_idxs, ridx)
                end
            end
        end 
    else 
