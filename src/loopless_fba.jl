...
"""
function loopless_fba_data(organism; time_limit=1800, silent=true, nullspace_formulation=false, type="loopless_fba", json=true, yeast=false, optimizer=SCIP.Optimizer, max_reactions=[], print_objective=false, scip_tol=1.0e-6, enzyme_data=false, save_model=false, seed=10, μ_mean=1.2)
    # build model
    if yeast 
    ...
    add_loopless_constraints(molecular_model, model, nullspace_formulation=nullspace_formulation, max_flux_bound, enzyme_data=enzyme_data)

    if save_model
        write_to_file(model, "../experiments/lp_models/gecko_" * organism * ".lp")
    end 
    ...
    if optimizer == SCIP.Optimizer
        MOI.set(model, MOI.RawOptimizerAttribute("numerics/feastol"), scip_tol)
    ...
    return objective_loopless_fba, vcat(x,a,G), time_loopless_fba, nodes, termination_loopless_fba, feasible
end
