...
"""
function loopless_fba_data(organism; time_limit=1800, silent=true, nullspace_formulation=false, type="loopless_fba", json=true, yeast=false, optimizer=SCIP.Optimizer, max_reactions=[], print_objective=false, scip_tol=1.0e-6, scip_seed=0)
    # build model
    if yeast 
    ...
    model = make_optimization_model(molecular_model, optimizer; scip_seed=scip_seed)
    ...
    file_name = joinpath("json/" * organism * "_" * type * "_" * string(time_limit) * "_scip_seed_" * string(scip_seed) * ".json")
    if json 
...
"""
function loopless_fba_blocked_data(organism; time_limit=180, ceiling=1000, same_objective=true, vector_formulation=true, shortest_cycles=false, block_limit=100, type="loopless_fba_blocked", nullspace_formulation=false, reduced=false, json=true, cff=false, cff_time_limit=1800, scip_seed=0)
    # load model
    molecular_model = load_model("../molecular_models/" * organism * ".json")
    # print_model(molecular_model, organism)
    ...
    if cff
        ...
        @show length(cycles)
        unbounded_cycles_original = [cycle[1] for cycle in cycles]
    ...
    dict[:objective_function] = objective(molecular_model)
    dict[:sense] = objective_sense(model)
    dict[:num_blocked_cycles] = num_blocked_cycles
    dict[:shortest_cycles] = shortest_cycles 
    dict[:block_limit] = block_limit
    dict[:scip_seed] = scip_seed

    if !same_objective
        file_name = joinpath("json/" * organism * "_" * type * "_" * string(time_limit) * "_" * string(ceiling) * "_custom_objective.json")
    else 
        if scip_seed != 0
            file_name = joinpath("json/" * organism * "_" * type * "_" * string(time_limit) * "_scip_seed_" * string(scip_seed) * "_" * string(ceiling) * ".json")
        else 
            file_name = joinpath("json/" * organism * "_" * type * "_" * string(time_limit) * "_" * string(ceiling) * ".json")
        end
    end
