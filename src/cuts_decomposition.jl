...
function compute_MIS(solution_a, S_int, solution_master, internal_rxn_idxs; fast=true, time_limit=1800, silent=true, multiple_mis=0, mis_solver=HiGHS.Optimizer)
    if !fast
        ...
        if mis_solver != HiGHS.Optimizer
            optimizer = optimizer_with_attributes(mis_solver) 
            mis_model = dualize(model, optimizer)
        else
...
function combinatorial_benders(master_problem, internal_rxn_idxs, S, lb, ub; max_iter=Inf, fast=true, time_limit=1800, silent=true, multiple_mis=0, big_m=false, save_model=false, subproblem_solver=HiGHS.Optimizer, mis_solver=HiGHS.Optimizer, indicator=false)
    ...
    @show multiple_mis
    @show fast
    println("objective function : ", objective_function(master_problem))
    println("objective sense : ", objective_sense(master_problem))
    println("--------------------------------------------------------")
    ...
    if save_model
        write_to_file(master_problem, "../experiments/lp_models/cb_master.lp")
    end 
    ...
    @assert !isnan(objective_value_master)
    ...
    # assert that ll FBA and solution of master problem are close
    if objective_sense(master_problem) == MOI.MAX_SENSE
        @assert objective_value_master <= objective_value_fba + 0.0001
    else 
        @assert objective_value_master >= objective_value_fba - 0.0001
    end
    ...
    while termination_sub == MOI.INFEASIBLE && iter < max_iter && time()-start_time_cb < time_limit
        ...
        if save_model
            write_to_file(master_problem, "../experiments/lp_models/master_problem_" * string(iter) * ".lp")
        end 
        ...
        @show length(C_list)
        if isempty(C_list)
            if mis_model_termination != MOI.INFEASIBLE
                ...
                feasible, _, _ = thermo_feasible_mu(non_zero_flux_indices, non_zero_flux_directions, S; scip_tol=0.001)
                @assert feasible
                sub_problem = Model(subproblem_solver)
                if !isinf(time_limit)
...
function combinatorial_benders_data(organism; time_limit=1800, json=true, max_iter=Inf, fast=true, silent=true, optimizer=SCIP.Optimizer, subproblem_solver=HiGHS.Optimizer, store_optimal_solution=false, scip_tol=1.0e-6, yeast=false, multiple_mis=0, big_m=false, indicator=true, enzyme_data=false, μ_mean=1.0, seed=1, mis_solver=HiGHS.Optimizer)
    ...
    if enzyme_data
        molecular_model = build_gecko_model(molecular_model, seed, μ_mean)
    end 
    ...
    # write_to_file(model, "../experiments/csv/models/cb_" * organism * ".lp")
    ...
    if termination == MOI.OPTIMAL
        thermo_feasible = is_feasible(master_problem.moi_backend.optimizer.model, x, a, S, internal_rxn_idxs, cuts, lb, ub, tol=0.0001, check_indicator=false)
    ...
    if optimizer != SCIP.Optimizer || subproblem_solver != HiGHS.Optimizer || mis_solver != HiGHS.Optimizer
        type = type * "_" * replace(string(optimizer), ".Optimizer"=>"")
        type = type * "_" * replace(string(subproblem_solver), ".Optimizer"=>"")
        type = type * "_" * replace(string(mis_solver), ".Optimizer"=>"")
    end
    if scip_tol != 1.0e-6
        type = type * "_tol_" * string(scip_tol)
    end
    ...
    return objective_value, termination, thermo_feasible, x
end
...
function is_feasible(o, solution_flux, solution_direction, S, internal_rxn_idxs, cuts, lb, ub; check_steady_state=true, check_bounds=true, check_thermodynamic_feasibility=true, check_cuts=true, check_indicator=true, tol=0.000001)
    ...
    if check_bounds
        if !solution_within_bounds(solution_flux, lb, ub, tol=tol)
            ...
            for (idx, val) in enumerate(solution_flux)
                if solution_flux[idx] < lb[idx] - tol || solution_flux[idx] > ub[idx] + tol
                    @show val, lb[idx], ub[idx]
                end
            end
            return false
        end
