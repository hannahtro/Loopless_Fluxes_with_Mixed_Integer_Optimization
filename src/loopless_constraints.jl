...
"""
function add_loopless_constraints(molecular_model, model, max_flux_bound=1000; nullspace_formulation=true, reduced=false, enzyme_data=false)
    # loopless model
...
"""
# ATTENTION: cylce should just contain internal reactions
...
"""
function thermo_feasible_mu(cycle, flux_directions, S, max_flux_bound=1000; scip_tol=1e-6, solution_dict=Dict(), file_name="report.txt")
    # warning if flux_directions not integral
    if sum([(i != 1 || i != 0) ? 0 : 1 for i in flux_directions]) != 0
        @warn "flux directions should be binary"
    end
    ...
    if status == MOI.OPTIMAL
        sol_G = MOI.get.(thermo_feasible_model, MOI.VariablePrimal(), G)
        sol_μ = MOI.get.(thermo_feasible_model, MOI.VariablePrimal(), μ)
    else 
        sol_G = NaN
        sol_μ = NaN
    end
    ...
    return status == MOI.OPTIMAL, sol_G, sol_μ
end
