...
def load_data(file='csv/results_bigg_SCIP.csv', big_m=False, indicator=True, mis_list=[], remove_easy_instances=False):
...
def build_solved_instances_plot(colors, linestyles, markerstyles, big_m=False, indicator=True, all_subplots=False, mis_list=[]):
    ...
    for idx, mis in enumerate(mis_list):
        if big_m:
            if mis == 0.5:
                axs[0].plot(data["time_cb_mis_" + str(mis)], data["instances_cb_mis_" + str(mis)], color=colors[idx], linestyle=linestyles[idx], linewidth=2.0)
            else:
